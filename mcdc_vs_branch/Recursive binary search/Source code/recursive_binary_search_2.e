...
feature
	...
	binary_search_recursive (a: SIMPLE_ARRAY [INTEGER]; value: INTEGER; bn: INTEGER): INTEGER
		...
		do
			if a.count > 0 then
				Result := binary_search_recursive_step (a, value, 2, a.count, bn)
			else
				Result := 0
			end
		ensure
		...
		do
			if lower >= upper then
				...
				elseif a [mid] > value then
					Result := binary_search_recursive_step (a, value, lower, mid - 1, bn)
						-- check a.sequence.interval (lower, upper) ~ a.sequence.interval(lower, mid-1) + a.sequence.interval (mid, upper) end
				else
					Result := binary_search_recursive_step (a, value, mid + 1, upper, bn)
				end
			end
		ensure
