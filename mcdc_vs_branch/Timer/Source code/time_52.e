...
feature
	...
	get_time: TIME_APP_52
		do
			Result := create {TIME_APP_52}.make_2 (hour, minute, second)
		ensure
	...
	later_than (a_time: TIME_APP_52; bn: INTEGER): BOOLEAN
		do
	...
	equals (a_object: ANY; bn: INTEGER): BOOLEAN
		...
		-- 	status: functional
		do
			Result := if attached {TIME_APP_52} a_object as a_time then
				(hour = a_time.get_hour and minute = a_time.get_minute and second = a_time.get_second)
			else
				False
			end
		ensure
			a_object_not_time: not attached {TIME_APP_52} a_object as a_time implies Result = False
			a_object_is_time: attached {TIME_APP_52} a_object as a_time implies Result = (hour = a_time.get_hour and minute = a_time.get_minute and second = a_time.get_second)
		end
feature {NONE} -- Ghost variables

	stop_minute (a_start, a_stop: TIME_APP_52; bn: INTEGER): INTEGER
		note
	...
	stop_hour (a_start, a_stop: TIME_APP_52; bn: INTEGER): INTEGER
		note
			status: functional, ghost
			explicit: wrapping, contracts
		require
	...
	diff_hour (a_start, a_stop: TIME_APP_52; bn: INTEGER): INTEGER
		note
			status: functional, ghost
			explicit: wrapping, contracts
		require
	...
	diff_minute (a_start, a_stop: TIME_APP_52; bn: INTEGER): INTEGER
		note
			status: functional, ghost
			explicit: wrapping, contracts
		require
	...
	diff_second (a_start, a_stop: TIME_APP_52; bn: INTEGER): INTEGER
		note
			status: functional, ghost
			explicit: wrapping, contracts
		require
...
feature
	...
	trusted_difference (a_start, a_stop: TIME_APP_52; bn: INTEGER): TIME_APP_52
		note
			explicit: wrapping, contracts
		require
			a_start_is_wrapped: a_start.is_wrapped
		...
		do
			l_diff := create {TIME_APP_52}.make
	...
	difference (a_start, a_stop: TIME_APP_52; bn: INTEGER): TIME_APP_52
		note
			explicit: wrapping, contracts
		require
			a_start_is_wrapped: a_start.is_wrapped
	...
	time_options (a_start, a_stop: TIME_APP_52; a_sel: INTEGER; bn: INTEGER): TIME_APP_52
		note
			status: impure
			explicit: wrapping, contracts
		require
