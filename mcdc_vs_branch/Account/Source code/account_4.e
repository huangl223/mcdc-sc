...
feature -- Basic operations
	...
	transfer (amount: INTEGER; other: ACCOUNT_4)
		...
		require
			amount_not_negative: amount >= 0
			amount_available: amount <= available_amount
			-- no_aliasing: other /= Current -- fault
		do
			withdraw (amount)
	...
	reset_transaction_code
		do
			-- code := 0 -- fault: missing instruction
		ensure
			code = 0
			modify_field (["code", "closed"], Current)
		end

	pay (amount: INTEGER; vendor: ACCOUNT_4; bn: INTEGER)
		...
		do
			...
			if amount > available_amount then -- branch1
				set_transaction_code (1) -- set_transaction_code (201) -- fault
